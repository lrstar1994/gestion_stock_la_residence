...
export async function getReception(id: string) {
  const { data, error } = await supabase.schema('stock')
    .from('receptions')
    .select('*, suppliers(*), locations(id, name), purchase_orders(id, reference, status, total_amount), cash_purchases(id, reference, status, amount_requested, amount_validated, amount_given, total_purchased, change_expected, change_returned, difference), receiver:profiles!receptions_created_by_fkey(id, full_name, role), validator:profiles!receptions_validated_by_fkey(id, full_name), reception_items(*, articles(id, name, families(id, name)), units(id, name, abbreviation), reception_anomalies(*)), reception_documents(*), reception_history(*, actor:profiles!reception_history_created_by_fkey(id, full_name))')
    .eq('id', id)
    .single()
